...
*/					itemWidth        : "140px",                     // 文件项的宽度
			...
			console.log("para.fileType::"+para.fileType);
			this.init = function(){
				this.createHtml();  // 创建组件html
				this.createCorePlug(para.fileType,para.projectPath);  // 调用核心js
			};
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '						<div class="uploadImg" style="width:'+imgWidth+'px;height:'+imgWidth+'px">';
			 ...
			 * 返回: 预览的html
			 */
			this.funDisposePreviewHtml = function(file, e,projectPath){
				var html = "";
				var imgWidth = parseInt(para.itemWidth.replace("px", ""))-15;
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = projectPath+"/multi_upload/control/images/fileType/";
				if(file.type.indexOf("rar") > 0){
					...
					fileImgSrc = fileImgSrc + "upload_file.png";
				}
			 ...
			 * 返回: 无
			 */
			this.createCorePlug = function(fileType,projectPath){
				console.log("createCorePlug:fileType"+fileType);
				var params = {
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								...
								reader.onload = function(e) {
									// 处理下配置参数和格式的html
									html += self.funDisposePreviewHtml(file, e,projectPath);
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<input  type='hidden' id='"+para.typeStr+"["+file.index+"]' value=" + response + "/>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
