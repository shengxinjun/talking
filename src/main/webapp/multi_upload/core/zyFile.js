...
var ZYFILE = {
		...
		// 上传多个文件
		funUploadFiles : function(e,fileType){
			console.log("funUploadFiles:"+fileType);
			flag = true;
			var self = this;  // 在each中this指向没个v  所以先将this保留
			// 遍历所有文件  ，在调用单个文件上传的方法
			$.each(this.uploadFile, function(k, v){
				if(v.name.indexOf(".")==-1){
					...
					if(fileType=='image'){
						var last = v.name.split(".")[1];
						if(last.toUpperCase()=="JPG"||last.toUpperCase()=="JPEG"||last.toUpperCase()=="PNG"||last.toUpperCase()=="GIF"||last.toUpperCase()=="BMP"){
						}else{
							flag = false;
						}
					}
		...
		// 初始化
		init : function(fileType){  // 初始化方法，在此给选择、上传按钮绑定事件
			...
			if(self.uploadInput){
				...
				this.uploadInput.addEventListener("click", function(e) {
					self.funUploadFiles(e,fileType); 
				}, false);	
			}
		}
};
